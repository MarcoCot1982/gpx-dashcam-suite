...
def ensure_backup(db_path, keep=2):
    base = os.path.basename(db_path)
    ts   = datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")
    dst  = os.path.join(os.path.dirname(db_path), f"{base}.backup.{ts}")
    with open(db_path, "rb") as rf, open(dst, "wb") as wf:
        wf.write(rf.read())
    # prune: keep only the <keep> most recent backup files for this db
    db_dir   = os.path.dirname(db_path)
    prefix   = f"{base}.backup."
    backups  = sorted(
        [f for f in os.listdir(db_dir) if f.startswith(prefix)],
        reverse=True   # newest first (timestamp in name sorts lexically)
    )
    for old in backups[keep:]:
        try:    os.remove(os.path.join(db_dir, old))
        except: pass
    return dst
...
class CacheEditor:
    ...
    def regeocode_gpx(self):
        ...
        geocoder_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "GPX_Geocoder.pyw")
        ...
        try:
            ...
            # Pass gpx_path as a bare positional arg — Geocoder reads it via sys.argv[1]
            subprocess.Popen([python_exe, geocoder_script, gpx_path],
                             cwd=os.path.dirname(os.path.abspath(__file__)))
            self.status_message(f"Geocoder launched for: {os.path.basename(gpx_path)}")
        except Exception as ex:
    ...
    def _load_db(self, path):
        """Open a DB file by path — shared by open_db() and auto-load on launch."""
        self.db_path = path
        try:
            if self.conn:
                try: self.conn.close()
            ...
            self._db_lbl.config(text=os.path.basename(path))
            self.status_message(f"DB opened: {os.path.basename(path)}")
            self.print_cache_stats()
        except Exception as ex:
...
def main():
    ...
    def _auto_load():
        # 1. Auto-load geocode_cache.db from the same folder as this script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        default_db = os.path.join(script_dir, "geocode_cache.db")
        if os.path.isfile(default_db):
            app._load_db(default_db)

        # 2. If --gpx <path> was passed, auto-load that GPX track
        gpx_path = None
        args = sys.argv[1:]
        if "--gpx" in args:
            idx = args.index("--gpx")
            if idx + 1 < len(args):
                gpx_path = args[idx + 1]
        elif len(args) == 1 and args[0] != "--gpx" and os.path.isfile(args[0]):
            # also accept a bare positional path (no flag)
            gpx_path = args[0]

        if gpx_path and os.path.isfile(gpx_path):
            # reuse the existing load_gpx_files machinery but with a known path
            app.gpx_tracks.clear()
            ...
            try:
                ...
                for ti, trk in enumerate(g.tracks):
                    for si, seg in enumerate(trk.segments):
                        ...
                        app.gpx_tracks.append({"file": gpx_path, "name": name, "points": points})
                        app.track_listbox.insert(tk.END, "  " + name)
                if app.gpx_tracks:
                    app.track_listbox.selection_set(0)
                    app.on_track_select()
                    app.status_message(
                        f"Auto-loaded: {os.path.basename(gpx_path)}  ·  "
                        f"{sum(len(t['points']) for t in app.gpx_tracks):,} points")
            except Exception as ex:
                app.status_message(f"Auto-load GPX failed: {ex}")

    # Small delay so the window is fully drawn before we touch the map widget
    root.after(200, _auto_load)
    root.mainloop()
